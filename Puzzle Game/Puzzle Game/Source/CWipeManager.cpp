...
HWND	CWipeManager::m_hWnd = NULL;
 const int CWipeManager::WIPEMANAGER_KIND_OF_IMAGE = 1;				//kind of image
 const int CWipeManager::WIPEMANAGER_LOAD_KIND_OF_IMAGE = 1;		//kind of load's image

CWipe	*CWipeManager::m_lpCWipe = NULL;							//for create wipe
GAMESCENE_NUMBER	CWipeManager::m_NextGameScene = GAMESCENE_NOTHING;	//next game scene
int		*CWipeManager::m_lpImage = NULL;							//for material of image
bool	CWipeManager::m_DirectionFlag = false;						//during wipe is directing that is flag

 CWipeManager::CWipeManager() {}
 
 CWipeManager::~CWipeManager() {
	 delete[]this->m_lpCWipe;				//for create wipe
	 this->m_lpCWipe = NULL;
	 delete[]this->m_lpImage;				//image
	 this->m_lpImage = NULL;
 }

void	CWipeManager::InitWipeManager() {

	//kind of image
	char	*lpImage[CWipeManager::WIPEMANAGER_KIND_OF_IMAGE] = {
		"Image\\Bg\\Black.bmp",
	};
	//allot each memory
	CWipeManager::m_lpImage = new int[CWipeManager::WIPEMANAGER_KIND_OF_IMAGE];			//image
	CWipeManager::m_lpCWipe = new CWipe[CWipeManager::WIPEMANAGER_KIND_OF_IMAGE];		//for create wipe's image

	//load material of image
	for (int i = 0; i < CWipeManager::WIPEMANAGER_KIND_OF_IMAGE; i++) { CWipeManager::m_lpImage[i] = CreateDDImage(lpImage[i], CWipeManager::m_hWnd); }
}

bool	CWipeManager::UpdateWipeManager() {

	//update wipe's direction. when wipe's direction was the end, get flag of the end
	if(CWipeManager::m_DirectionFlag) CWipeManager::m_DirectionFlag = CWipeManager::m_lpCWipe[0].UpdateWipe();
	if (CWipe::m_ChangeScene) {	//when return is true, change transiton to next scene
		CSceneManager::SetNextScene(this->m_NextGameScene);
		CWipe::m_ChangeScene = false;					//flag of the transition to scene into false
	}
	return CWipeManager::m_DirectionFlag;				//return is flag of transition to scene
}

void	CWipeManager::DrawWipeManager() {

	//wipe draw a while wipe is directing
	if (CWipeManager::m_DirectionFlag) CWipeManager::m_lpCWipe[0].DrawChara();
}

void	CWipeManager::ReleaseWipeManager() {

	for (int i = 0; i < CWipeManager::WIPEMANAGER_KIND_OF_IMAGE; i++) { ReleaseDDImage(CWipeManager::m_lpImage[i]); }
}

void	CWipeManager::CreateWipeManager(GAMESCENE_NUMBER next, tagWIPEMANAGER_DIRECT direct) {
	
	//get next game scene
	CWipeManager::m_NextGameScene = next;

	CWipeManager::m_DirectionFlag = true;			//flag of direction

	//each wipes' initializer
	TPOINT<int> wipeSize = { SCREEN_WIDTH, SCREEN_HEIGHT };
	TPOINT<float> wipePos = { 0 };
	for (int i = 0; i < CWipeManager::WIPEMANAGER_KIND_OF_IMAGE; i++) {
		CWipeManager::m_lpCWipe[i].InitChara(wipePos, wipeSize, direct);
		CWipeManager::m_lpCWipe[i].SetCharaImg(CWipeManager::m_lpImage[0]);
	}
}
/*
//************************************************************************************************
