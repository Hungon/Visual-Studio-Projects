...
int g_display_mode = MODE_FULLSCREEN;
//画像を扱うサーフェイス配列
IMAGESURFACE g_img[IMAGE_MAX] = {0};
//文字体を扱う
char* g_Font = FONT_GOS;
...
bool InitDDFullScreen(HWND hWnd){
	...
	g_display_mode = MODE_FULLSCREEN;
...
void UpdatePrimary(HWND hWnd){
	...
	//▼現在のモードがフルスクリーンモードなら
	if(g_display_mode == MODE_FULLSCREEN){
		//プライマリサーフェイスのフリップ
...
void FillBackSurface(){
	...
	//▼画面を塗りつぶす
	ddbltfx.dwFillColor = DDRGB(255,255,255);
...
void ChangeDisplayMode(HWND hWnd){
	...
	//画面のモードを交互に変更する
	pDisplayMode[g_display_mode](hWnd);
	//画像の再読み込み
	for(i = 0;i < IMAGE_MAX;i++){
...
void DDScaleBltSelectIndicate( float dst_x , float dst_y , int w , int h , int id , int src_x , int src_y , float pow , tagDDSCALESTARTINGPOINT dstPointFlag ){
	...
	switch( dstPointFlag ){
		//左上を基点
		case DDSCALE_STARTING_POINT_LEFT_TOP:
			//拡大・縮小後の転送先矩形を作成
			SetRect( &dst , (int)dst_x , (int)dst_y ,
				(int)( dst_x + scale_w ) , (int)( dst_y + scale_h ) );
			break;
		//中心を基点
		case DDSCALE_STARTING_POINT_CENTER:
			//拡大・縮小後の転送先矩形を作成
...
int CreateDDOffScreen(int w,int h){
	...
	//透過色の指定
	ZeroMemory(&ddck,sizeof(ddck));
	ddck.dwColorSpaceHighValue = DDRGB(0,50,0);		//透過色は自由
	ddck.dwColorSpaceLowValue = DDRGB(0,50,0);
